function [Sys, Mesh] = AssembHBKerr(Sys, Mesh)
FlagABC = false;
FlagDir = false;
FlagNeu = false;
FlagDD = false;
...
nums = CalcOrderMatSize(Sys.pOrd);
...
%     fprintf('+');
    ...
    if Mesh.elab(ie) == Mesh.NLlab % isfield(Mesh, 'kerr')
        if isfield(Sys,'SinOnly')
            ...
            for jh=1:nHarms
                sol = Sys.u0(gIs+(jh-1)*Sys.NDOFs);
                Field.E(jh,1) = Shape2(.5,.5)*sol;
            end
        end
        Mtrl.func = @(x)(Mesh.epsr(Mesh.elab(ie))+Mesh.kerr(Mesh.elab(ie))*abs(x).^2);
        [Mtrl, Field] = GetECouplFFT(Mtrl, Field);
        if isfield(Sys,'SinOnly')
            EpsrC = Mtrl.D(1:2:2*nHarms,1:2:2*nHarms);
        else
            EpsrC = Mtrl.D;
        end
        NurC = eye(nHarms);
        ...
        for kh = 1:nHarms 
            for j=1:nums
                for k = 1:nums
                    ...
                    XXT(tmpIdx) = Sys.Harms(jh)^2*T(j,k)*EpsrC(jh,kh);
                end
            end
        end
    end
