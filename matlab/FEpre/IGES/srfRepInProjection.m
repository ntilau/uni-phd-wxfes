function [nrmlmodel,srfbounds,UVb,nrmlb,dirb,srfindb,srfs,UVs,nrmls,srfinds]=srfRepInProjection(ParameterData,R,normal,pdir,sdir,dp,nppos,npneg,nspos,nsneg,model,UV,srfind,dpfactor,dpsfactor)
...
% Input:
%
% See help of projpartIGES for more information about the input variables
...
for j=1:np
    for i=1:ns
        ...
        if sind>0
            ...
            elseif or(ParameterData{sind}.type==128,ParameterData{sind}.type==144)
                [Ptmp,Ptmpu,Ptmpv]=nrbevalIGES(ParameterData{sind}.nurbs,UV(:,mind),ParameterData{sind}.dnurbs);
        ...
        if ParameterData{i}.type==144
            ...
            if ParameterData{i}.original
                ...
                if ParameterData{entiall}.type==128
                    ...
                    [P,isSCP,isSup,TRI,UVons]=retSrfCrvPnt(1,ParameterData,1,i,200,0);
                    
                    UVtmp=getInsideSurfacePoints(P,UVons,TRI,srfsdensity);
                    [Ptmp,Ptmpu,Ptmpv]=nrbevalIGES(ParameterData{entiall}.nurbs,UVtmp,ParameterData{entiall}.dnurbs);
                    ...
                    if numPtmp>0
                        ...
                        nrmlb(1,stsind:(stsind+numPtmp-1))=Ptmpu(2,isIn).*Ptmpv(3,isIn)-Ptmpu(3,isIn).*Ptmpv(2,isIn);
                        nrmlb(2,stsind:(stsind+numPtmp-1))=Ptmpu(3,isIn).*Ptmpv(1,isIn)-Ptmpu(1,isIn).*Ptmpv(3,isIn);
                        nrmlb(3,stsind:(stsind+numPtmp-1))=Ptmpu(1,isIn).*Ptmpv(2,isIn)-Ptmpu(2,isIn).*Ptmpv(1,isIn);
                ...
                if and(ParameterData{entiall}.previous_type==120,ParameterData{entiall}.well)
                    ...
                    Pu=nrbSrfRegularEvalIGES(ParameterData{i}.nurbs,ParameterData{i}.u(1),ParameterData{i}.u(2),ntest,ParameterData{i}.v(1),ParameterData{i}.v(2),2);
                    Pv=nrbSrfRegularEvalIGES(ParameterData{i}.nurbs,ParameterData{i}.u(1),ParameterData{i}.u(2),2,ParameterData{i}.v(1),ParameterData{i}.v(2),ntest);
                    ...
                    if or(any(isInside(Pu,pdir,sdir,refPpdir,refPsdir,deltap,deltas)),any(isInside(Pv,pdir,sdir,refPpdir,refPsdir,deltap,deltas)))
                        ...
                        for j=1:4
                            if le(j)>0
                                ...
                                if j==1
                                    ...
                                    [Ptmp,Ptmpu,Ptmpv]=nrbevalIGES(ParameterData{i}.nurbs,UVtmp,ParameterData{i}.dnurbs);
                                    dirstmp=Ptmpu;
                                elseif j==2
                        ...
                        nuv=25;
                        [P,UVons,TRI]=nrbSrfRegularEvalIGES(ParameterData{i}.nurbs,ParameterData{i}.u(1),ParameterData{i}.u(2),nuv,ParameterData{i}.v(1),ParameterData{i}.v(2),nuv);
...
function UVtmp=getInsideSurfacePoints(P,UV,TRI,srfsdensity)
...
UVtmp=zeros(2,numUVtot);

stind=0;

for i=1:mTRI
    if numUV(i)>0
        vec1(1:2)=UV(:,TRI(i,2))-UV(:,TRI(i,1));
        vec2(1:2)=UV(:,TRI(i,3))-UV(:,TRI(i,1));
        for j=1:numUV(i)
            rnd1=rand;
            rnd2=rand;
            if rnd1+rnd2>1
                UVtmp(:,stind+j)=UV(:,TRI(i,1))+(1-rnd1)*vec1(1:2)+(1-rnd2)*vec2(1:2);
            else
                UVtmp(:,stind+j)=UV(:,TRI(i,1))+rnd1*vec1(1:2)+rnd2*vec2(1:2);
            end
        end
        stind=stind+numUV(i);
    end
